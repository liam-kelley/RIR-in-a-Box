...
def json2obj(current_json_file_name, json_folder, future_path, save_path, model_info):
    '''
    Lifted from GAMMA-UMD/GWA/tools/json2obj.py on github
    '''
    with open(json_folder+'/'+current_json_file_name, 'r', encoding='utf-8') as file:
        data = json.load(file)
        model_jid = []
        model_uid = []
        model_bbox= []
        ...
        mesh_faces = []
        mesh_types = []
        save_folder = os.path.join(save_path, current_json_file_name[:-5])
        ...
        try:
            for r in room:
                ...
                for c in children:
                    ...
                    try:
                        ...
                        if os.path.exists(future_path+'/' + model_jid[idx]):
                            v, vt, _, faces, ftc, _ = igl.read_obj(future_path+'/' + model_jid[idx] + '/raw_model.obj')
            ...
            print(f'failed on {current_json_file_name}: {e}')
            return None
        ...
        if len(meshes) > 0:
            ...
            return str(save_folder + '/house.obj')
            # write_meshes_with_mtl(save_folder + '/house_struct.obj', structure_meshes)
        else:
            print(f'failed on {current_json_file_name}: "meshes" is empty')
            return None
